...
mypeer.on('connection', function(conn) {//connはdataconnection型オブジェクト
	...
	conn.on("data",function(data){
		...
		if(conn.label == "chat"){
			console.log("チャットテスト2");
			writeChat(conn.metadata, data);
			...
			console.log(data);
			 if (data.constructor === ArrayBuffer) {//ここら辺がまだよくわかってないです。
				...
				writeChat(conn.metadata, "<a download='" + conn.metadata + "' target='_blank' href='" + url + "'>" + conn.metadata + "</a>");
			}
...
function advanceConnect(mode, id, data){
	...
	if(mode == "chat") {
		...
		dataconn.on("open",function(){//ちゃんと戻ってきて生成に成功した場合
			console.log("チャットの送信が成功しました。");
			if(document.getElementById("chat_me").value != "")	dataconn.send(document.getElementById("chat_me").value);//相手に送る
		})
	//ファイルを送信する
	}else if(mode == "file") {
